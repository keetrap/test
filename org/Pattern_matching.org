...
#+begin_src bash
grep 'the' RegEx_example.txt  #This command will print all the lines contains word "the"
...
+ Using ~^~ we can match line staring with some character.
  #+begin_src bash
grep '^b' RegEx_example.txt
bob123
#+end_src

+ Using ~$~ we can match line ending with some character.
  #+begin_src bash
grep 'm$' RegEx_example.txt
info@example.com
user123@gmail.com
#+end_src

**** Wildcard ~.~
The dot (.) in regular expressions is a wildcard that matches any single character except for a newline.
#+begin_src bash
grep '.' RegEx_example.txt # This will match any line that contains at least one character.

grep 'c.m' RegEx_example.txt
~ info@example.com   # It matched with com in email.
~ user123@gmail.com
~ December 5, 2022   # It matched with cem in December
#+end_src

#+begin_src bash
grep '.s' RegEx_example.txt # This will match any chracter followed by s
#+end_src

#+begin_src bash
grep '\.' RegEx_example.txt # This will match a literal dot (.) in the lines
Mountain peaks touch the sky.
Mountain trails wind through the forest.
Ocean waves crash on the shore.
The sun sets, coloring the sky.
...
grep '\.$' RegEx_example.txt # This will match the lines ending with dot (.)
Mountain peaks touch the sky.
Mountain trails wind through the forest.
Ocean waves crash on the shore.
The sun sets, coloring the sky.
